...
keyQuestions:
  ...
  - How can teams improve report quality before increasing runtime?
  - Which source, scope, and output constraints should be defined up front?
notFor:
  - One-line search prompts with no decision or source requirements
  - Teams that only need deterministic extraction from known internal data
  - Readers looking for generic prompt-writing tips without research operations
updateTriggers:
  - New deep research product capabilities or source-collection workflows
  - Changes in source policy, citation expectations, or report consumers
  - Repeated weak reports caused by vague scope or unclear evidence requirements
