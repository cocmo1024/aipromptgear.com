...
keyQuestions:
  - How should deep research systems control runtime and cost?
  - Which budget dimensions matter beyond token spend?
  - When should a deep research run stop instead of searching more?
notFor:
  - Simple internal knowledge-base answers that do not require research
  - Teams with no visibility into runtime, search breadth, or model spend
  - Readers looking only for generic cloud cost management
updateTriggers:
  - New model pricing, search-tool pricing, or background processing behavior
  - Changes in deep research product tiers or promised turnaround times
  - Repeated incidents where research runs exceeded useful cost or latency
