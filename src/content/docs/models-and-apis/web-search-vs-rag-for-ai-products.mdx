...
keyQuestions:
  - When should an AI product use web search instead of RAG?
  - When is owned retrieval safer than live external search?
  - What does a hybrid search architecture add in cost, latency, and review burden?
notFor:
  - Teams looking for a generic search-engine optimization checklist
  - Products that only need fixed internal policy or help-center answers
  - Readers comparing vector databases without product workflow context
updateTriggers:
  - Major provider changes to built-in web search tools
  - Pricing or latency changes that alter the economics of search-enabled workflows
  - New evidence quality requirements for source-grounded AI products
