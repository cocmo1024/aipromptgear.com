...
keyQuestions:
  - How are public image prompt examples adapted into reusable prompt patterns?
  - What does attribution mean inside the image prompt archive?
  - Which controls and failure modes should readers inspect before reusing a pattern?
notFor:
  - Readers looking for raw copied prompt dumps without attribution
  - Teams that want to mirror public prompt text verbatim
  - Users who need model-specific legal advice about training data or image rights
updateTriggers:
  - New image prompt families or attribution conventions in the archive
  - Changes in image-generation model behavior around text, references, or layout control
  - New public source collections that require clearer adaptation policy
...
## How to use a pattern responsibly
