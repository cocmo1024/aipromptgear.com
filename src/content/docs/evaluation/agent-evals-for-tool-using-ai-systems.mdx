...
notFor:
  - Teams evaluating only single-turn chatbot answers
  - Readers looking for a leaderboard-style benchmark with no production traces
  - Workflows where the model never calls tools, changes state, or asks for approval
updateTriggers:
  - New tool-use capabilities that change how traces or approvals should be scored
  - Production incidents caused by wrong tool choice, bad arguments, or unsafe side effects
  - Changes in evaluation ownership, release gates, or live-sampling policy
