...
targetRoles:
  ...
  - Product leads
keyQuestions:
  - How should teams evaluate AI agent tool selection?
  - Which failure classes explain wrong tool use, missing tool use, and approval misses?
  - How can tool-selection evals route failures to the right owner?
notFor:
  - Chatbot answer-quality evals with no tool calls
  - Teams with no trace capture or tool-call logging
  - Readers looking only for generic benchmark accuracy metrics
updateTriggers:
  - New tool types, tool schemas, approval patterns, or trace-grading practices
  - Production incidents involving wrong tool calls or missed approval boundaries
  - Model or orchestration changes that alter tool-selection behavior
